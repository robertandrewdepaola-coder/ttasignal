...
def parse_analysis_for_dashboard(ai_analysis: str, ticker: str, current_price: float, level_A: float, level_B: float, timeframe: str = "Daily", mtf_data: dict = None) -> dict:
    ...
    if not final_verdict or len(final_verdict) < 20:
        if trigger_state == "BEARISH":
            ...
            # v16.17 MTF FIX - Use actual traffic light data from app.py
            if mtf_data:
                # Convert dot colors to status (green=STRONG, yellow=WEAK, red=AVOID)
                def color_to_status(color):
                    if not color or color == '6b7280':
                        return "STRONG"  # Gray/no data = default allow
                    c = str(color).lower()
                    if '00e676' in c or c == 'green':
                        return "STRONG"  # Green
                    elif 'fbbf24' in c or 'f59e0b' in c or c == 'yellow':
                        return "WEAK"    # Yellow
                    else:
                        return "AVOID"   # Red or other
                ...
                # Fallback if mtf_data not provided
